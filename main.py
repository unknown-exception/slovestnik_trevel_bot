...
def print_tours(trs: [tours.Tour]):
    ...
    for t in trs:
        urlButton1 = InlineKeyboardButton(
            text=t.head, callback_data="order_tour_" + t.id
        )
...
async def new_member(message: types.Message):
    await message.reply(
        "Приветствуем, на какие мероприятия вы хотите записаться?",
...
async def check_button(call: types.CallbackQuery):
    ...
    await call.message.answer(
        f"""Вы хотите записаться на [{tours.get_tour_by_id(tour).head}]
...
async def hiking_list(message: types.Message):
    markup = print_tours(tours.get_hike())
    await message.answer(
        f"""Расписание по запросу {message.text}
...
@dp.message_handler(regexp="Aвтобусные туры")
async def bus_list(message: types.Message):
...
async def echo123(message: types.Message):
    if not message.chat.id in tmp_order:
        await message.answer(
            "Я не знаю такой команды, извините", reply_markup=MAIN_KEYBOARD
        )
    else:
